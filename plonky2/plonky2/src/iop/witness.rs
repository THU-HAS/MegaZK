...
impl<'a, F: Field> PartitionWitness<'a, F> {
    ...
    pub fn get_rep_index(&mut self, target: Target) -> usize {
        self.representative_map[self.target_index(target)]
    }
    ...
    pub fn full_witness_flat(self) -> Vec<F> {
        // let mut wire_values = vec![F::ZERO; self.degree * self.num_wires * 8];
        let mut wire_values = vec![F::ZERO; self.degree * self.num_wires];
        for i in 0..self.degree {
            for j in 0..self.num_wires {
                let t = Target::Wire(Wire { row: i, column: j });
                if let Some(x) = self.try_get_target(t) {
                    // wire_values[j * self.degree * 8 + i] = x;
                    wire_values[j * self.degree + i] = x;
                }
            }
        }
