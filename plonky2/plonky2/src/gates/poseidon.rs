    ...
    for PoseidonGenerator<F, D>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        for i in 0..4 {
            output_list.push(Target::wire(self.row, PoseidonGate::<F, D>::wire_delta(i)));
        }

        for r in 1..poseidon::HALF_N_FULL_ROUNDS {
            for i in 0..SPONGE_WIDTH {
            output_list.push(Target::wire(self.row, PoseidonGate::<F, D>::wire_full_sbox_0(r, i)));
            }
        }

        for r in 0..poseidon::N_PARTIAL_ROUNDS {
            output_list.push(Target::wire(self.row, PoseidonGate::<F, D>::wire_partial_sbox(r)));
        }

        for r in 0..poseidon::HALF_N_FULL_ROUNDS {
            for i in 0..SPONGE_WIDTH {
            output_list.push(Target::wire(self.row, PoseidonGate::<F, D>::wire_full_sbox_1(r, i)));
            }
        }

        for i in 0..SPONGE_WIDTH {
            output_list.push(Target::wire(self.row, PoseidonGate::<F, D>::wire_output(i)));
        }
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.push(self.row * num_wires);
    }
