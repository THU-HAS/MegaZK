    ...
    for InterpolationGenerator<F, D>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        self.gate.wires_shifted_evaluation_point()
            .chain(
                (0..self.gate.num_intermediates()).flat_map(|i| {
                    self.gate.wires_intermediate_eval(i)
                        .chain(self.gate.wires_intermediate_prod(i))
                })
            )
            .chain(self.gate.wires_evaluation_value())
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        ...
        f_params.extend(self.interpolation_domain.iter().chain(self.gate.barycentric_weights.iter()));
    }
