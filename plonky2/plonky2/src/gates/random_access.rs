    ...
    for RandomAccessGenerator<F, D>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        vec![local_target(self.gate.wire_claimed_element(self.copy))]
            .into_iter()
            .chain(
                (0..self.gate.bits).map(|i| local_target(self.gate.wire_bit(i, self.copy)))
            )
            .collect()
    }
