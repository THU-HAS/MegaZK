...
#[cfg(feature = "gpu")]
use crate::integration::orchestrator::GpuProverOrchestrator;
#[cfg(feature = "gpu")]
...
#[cfg(feature = "gpu")]
use crate::plonk::circuit_data::GpuProverOnlyCircuitData;
...
use crate::util::{ceil_div_usize, debug_dump, log2_ceil, transpose};
    ...
    C::InnerHasher: Hasher<F>,
{
    ...
    println!("Time taken to generate partial witness: {:?}", s.elapsed());

    prove_with_partition_witness(prover_data, common_data, partition_witness, timing)
}

#[cfg(feature = "gpu")]
pub(crate) fn prove_gpu<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    prover_data: &GpuProverOnlyCircuitData<F, C, D>,
    _common_data: &CommonCircuitData<F, D>,
    prover_setup: &mut GpuProverSetup<F, D>,
    inputs: &[F],
    timing: &mut TimingTree,
) -> Result<ProofWithPublicInputs<F, C, D>>
where
    ...
    C::InnerHasher: Hasher<F>,
{
    let (circuit, workspace, proof_state, proof_out) = prover_setup.parts_mut();
    GpuProverOrchestrator::prove::<F, C, D>(
        circuit,
        workspace,
        proof_state,
        proof_out,
    ...
    C::InnerHasher: Hasher<F>,
{
    ...
    debug_dump::write_json("public_inputs_ref.json", || {
        serde_json::to_vec(&public_inputs)
    });
    ...
    println!("Time taken to generate full witness: {:?}", s.elapsed());
    ...
    println!("Time taken to commmit witness: {:?}", s.elapsed());

    debug_dump::write_json("cm1cap_ref.json", || {
        serde_json::to_vec(&wires_commitment.merkle_tree.cap.0)
    });
    ...
    debug_dump::write_json("betas_ref.json", || serde_json::to_vec(&betas));
    debug_dump::write_json("gammas_ref.json", || serde_json::to_vec(&gammas));
    ...
    println!("Time taken to calculate pp: {:?}", s.elapsed());

    debug_dump::write_json("zs_ref.json", || {
        let zs_flatten = zs_partial_products_lookups
            .par_iter()
            .map(|v| v.values.clone())
            .collect::<Vec<_>>()
            .concat();
        serde_json::to_vec_pretty(&zs_flatten)
    });
    ...
    println!("Time taken to commit pp: {:?}", s.elapsed());

    debug_dump::write_json("cm2cap_ref.json", || {
        serde_json::to_vec(&partial_products_zs_and_lookup_commitment.merkle_tree.cap.0)
    });
    ...
    debug_dump::write_json("alphas_ref.json", || serde_json::to_vec(&alphas));
    ...
    println!("Time taken to calculate quotient: {:?}", s.elapsed());
    debug_dump::write_json("qs_ref.json", || serde_json::to_vec(&quotient_polys));
    let s = Instant::now();
    let quotient_polys: Vec<PolynomialCoeffs<F>> = quotient_polys
        .into_par_iter()
        .map(PolynomialValues::new)
        .map(|values| values.coset_ifft(F::coset_shift()))
        .collect();
    println!(" intt: {:?}", s.elapsed().as_millis());
    ...
    println!(" chunk: {:?}", s0.elapsed().as_millis());
    // println!("size of quotient poly chunks (to commit): ({:?}, {:?})", all_quotient_poly_chunks.len(), all_quotient_poly_chunks[0].coeffs.len());
    ...
    println!("Time taken to commmit quotient: {:?}", s.elapsed());

    debug_dump::write_json("cm3cap_ref.json", || {
        serde_json::to_vec(&quotient_polys_commitment.merkle_tree.cap.0)
    });
    ...
    debug_dump::write_text("openings_ref.json", || format!("{:?}", openings));
    if cfg!(feature = "verbose") {
        println!("Before observing openings: {:?}", &challenger.sponge_state);
        println!("Input buffer: {:?}", &challenger.input_buffer)
    }

    challenger.observe_openings(&openings.to_fri_openings());
    if cfg!(feature = "verbose") {
        println!("After observing openings: {:?}", &challenger.sponge_state)
    }
    let instance = common_data.get_fri_instance(zeta);
    println!("Time taken to get openings: {:?}", s.elapsed());
    ...
    println!("Time taken to prove openings: {:?}", s.elapsed());
    ...
    let a = Ok(ProofWithPublicInputs::<F, C, D> {
        proof,
        public_inputs,
    });
    debug_dump::write_text("proof_ref.json", || format!("{:?}", a));
...
>(
    ...
    let quotient_values: Vec<Vec<F>> = points_batches
        ...
        .flat_map(|(batch_i, xs_batch)| {
            ...
            for (&i, &x) in indices_batch.iter().zip(xs_batch) {
                ...
                // if batch_i == 0 && i == 13 {
                //     println!("{:?}", local_constants_sigmas);
                //     println!("{:?}", local_zs);
                // }
            }
    ...
    // let points_batches = points.chunks(BATCH_SIZE);
    // let num_batches = ceil_div_usize(points.len(), BATCH_SIZE);
    // let mut quotient_values: Vec<Vec<F>> = Vec::with_capacity(num_batches);
    //     for (batch_i, xs_batch) in points_batches.into_iter().enumerate() {
    //         // Each batch must be the same size, except the last one, which may be smaller.
    //         debug_assert!(
    //             xs_batch.len() == BATCH_SIZE
    //                 || (batch_i == num_batches - 1 && xs_batch.len() <= BATCH_SIZE)
    ...
    //                 .for_each(|v| *v *= denominator_inv);
    //         }
    //         quotient_values.push(quotient_values_batch.into_iter().flatten().collect());
    //     }

    debug_dump::write_json("qs_ref.json", || {
        serde_json::to_vec(&transpose(&quotient_values))
    });
    if debug_dump::enabled() {
        println!(
            "size of quotient values (in file): ({:?}, {:?})",
            quotient_values.len(),
            quotient_values[0].len()
        );
    }

    let tmp = transpose(&quotient_values);
    println!(" big map: {:?}", s.elapsed().as_millis());
    // let s = Instant::now();
    // let tmp = tmp
    //     .into_par_iter()
    //     .map(PolynomialValues::new)
