...
#[cfg(feature = "gpu")]
use crate::integration::PolyLayout;
...
#[cfg(feature = "gpu")]
pub struct GpuCircuitData<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>
{
    pub prover_only: GpuProverOnlyCircuitData<F, C, D>,
    pub verifier_only: VerifierOnlyCircuitData<C, D>,
    pub common: CommonCircuitData<F, D>,
    prover_setup: GpuProverSetup<F, D>,
}
...
{
    pub(crate) fn new(
        prover_only: GpuProverOnlyCircuitData<F, C, D>,
    ...
    pub fn prove(&mut self, witness_inputs: &[F]) -> Result<ProofWithPublicInputs<F, C, D>> {
        let expected = self.prover_setup.witness_input_count();
        ensure!(
            witness_inputs.len() == expected,
            "GPU witness input length mismatch: expected {}, got {}",
            expected,
            witness_inputs.len()
        );
        prove_gpu::<F, C, D>(
...
#[cfg(feature = "gpu")]
pub struct GpuProverOnlyCircuitData<
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
    const D: usize,
> {
