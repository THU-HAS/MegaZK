...
#[cfg(feature = "gpu")]
use cudart::memory::memory_copy_async_w_offset;
#[cfg(feature = "gpu")]
...
use crate::gates::selectors::{
    selector_ends_lookups, selector_polynomials, selectors_lookup, SelectorsInfo,
};
use crate::hash::hash_types::{HashOut, HashOutTarget, MerkleCapTarget, RichField};
use crate::hash::merkle_proofs::MerkleProofTarget;
use crate::hash::merkle_tree::MerkleCap;
#[cfg(feature = "gpu")]
use crate::integration::top::GpuProverSetup;
...
#[cfg(feature = "gpu")]
use crate::plonk::circuit_data::{GpuCircuitData, GpuProverOnlyCircuitData};
...
use crate::util::{debug_dump, log2_ceil, log2_strict, transpose, transpose_poly_values};
...
struct PreprocessedPolynomials<F: Field> {
    constant_vecs: Vec<PolynomialValues<F>>,
    sigma_vecs: Vec<PolynomialValues<F>>,
    fft_root_table: FftRootTable<F>,
}

struct PermutationPreprocessing<F: Field> {
    forest: Forest,
    subgroup: Vec<F>,
    k_is: Vec<F>,
}

struct CommonDataInputs<F: RichField + Extendable<D>, const D: usize> {
...
struct SharedPreprocessing<F: RichField + Extendable<D>, const D: usize> {
    geometry: BuildGeometry,
    polynomials: PreprocessedPolynomials<F>,
    permutation: PermutationPreprocessing<F>,
    common_inputs: CommonDataInputs<F, D>,
}

struct SharedProverInputs<F: RichField + Extendable<D>, const D: usize> {
    generators: Vec<WitnessGeneratorRef<F, D>>,
...
struct CircuitDigestInputs<F: Field> {
    domain_separator: Vec<F>,
    degree_bits: usize,
}

struct CommonBuildArtifacts<F: RichField + Extendable<D>, const D: usize> {
    common: CommonCircuitData<F, D>,
    prover_inputs: SharedProverInputs<F, D>,
    constant_vecs: Vec<PolynomialValues<F>>,
    digest_inputs: CircuitDigestInputs<F>,
    success: bool,
}

struct CpuCommitment<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize> {
    constants_sigmas: PolynomialBatch<F, C, D>,
}
...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
    ...
    fn preprocess_shared<C: GenericConfig<D, F = F>>(
        &mut self,
        timing: &mut TimingTree,
        sigma_dump_name: &'static str,
    ) -> SharedPreprocessing<F, D> {
        ...
        debug_dump::write_json(sigma_dump_name, || {
            let sigma = sigma_vecs
                .par_iter()
                .map(|v| v.values.clone())
                .collect::<Vec<_>>()
                .concat();
            serde_json::to_vec(&sigma)
        });
        ...
        SharedPreprocessing {
            ...
            polynomials: PreprocessedPolynomials {
                constant_vecs,
                sigma_vecs,
                fft_root_table,
            },
            permutation: PermutationPreprocessing {
                forest,
                subgroup,
                k_is,
            },
            common_inputs: CommonDataInputs {
                fri_params,
                gates,
                selectors_info,
    ...
    fn commit_cpu_constants_sigmas<C: GenericConfig<D, F = F>>(
        preprocessing: &mut SharedPreprocessing<F, D>,
        commit_to_sigma: bool,
        timing: &mut TimingTree,
    ) -> CpuCommitment<F, C, D> {
        let constants_sigmas = if commit_to_sigma {
            let constants_sigmas_vecs = [
                mem::take(&mut preprocessing.polynomials.constant_vecs),
                preprocessing.polynomials.sigma_vecs.clone(),
            ]
            .concat();
            debug_dump::write_json("cm0data_ref.json", || {
                let values = constants_sigmas_vecs
                    .par_iter()
                    .map(|v| v.values.clone())
                    .collect::<Vec<_>>()
                    .concat();
            ...
            PolynomialBatch::<F, C, D>::from_values(
                ...
                preprocessing.geometry.rate_bits,
                PlonkOracle::CONSTANTS_SIGMAS.blinding,
                preprocessing.geometry.cap_height,
                timing,
                Some(&preprocessing.polynomials.fft_root_table),
            )
        } else {
            PolynomialBatch::<F, C, D>::default()
        };

        debug_dump::write_json("cm0cap_ref.json", || {
            serde_json::to_vec(&constants_sigmas.merkle_tree.cap.0)
        });
        debug_dump::write_json("cm0leaves_ref.json", || {
            serde_json::to_vec(&constants_sigmas.merkle_tree.leaves)
        });

        CpuCommitment { constants_sigmas }
    }

    fn build_common_artifacts(
        mut self,
        preprocessing: SharedPreprocessing<F, D>,
    ) -> CommonBuildArtifacts<F, D> {
        let SharedPreprocessing {
            geometry,
            polynomials,
            permutation,
            common_inputs,
        ...
        for (i, generator) in self.generators.iter().enumerate() {
            for watch in generator.0.watch_list() {
                let watch_index = permutation.forest.target_index(watch);
                let watch_rep_index = permutation.forest.parents[watch_index];
        ...
        let num_gate_constraints = common_inputs
            .gates
            .iter()
        ...
        let num_partial_products = num_partial_products(
            ...
            common_inputs.quotient_degree_factor,
        );
        let lookup_degree = self.config.max_quotient_degree_factor - 1;
        let num_lookup_polys = if common_inputs.num_luts == 0 {
            0
        } else {
        ...
        let common = CommonCircuitData {
            config: self.config,
            fri_params: common_inputs.fri_params,
            gates: common_inputs.gates,
            selectors_info: common_inputs.selectors_info,
            quotient_degree_factor: common_inputs.quotient_degree_factor,
            num_gate_constraints,
            num_constants: common_inputs.num_constants,
            num_public_inputs: common_inputs.num_public_inputs,
            k_is: permutation.k_is,
            num_partial_products,
            num_lookup_polys,
            num_lookup_selectors: common_inputs.num_lookup_selectors,
            luts: self.luts,
        };
        ...
        CommonBuildArtifacts {
            common,
            prover_inputs: SharedProverInputs {
                generators: self.generators,
                generator_indices_by_watches,
                sigma_vecs: polynomials.sigma_vecs,
                subgroup: permutation.subgroup,
                public_inputs: self.public_inputs,
                representative_map: permutation.forest.parents,
                fft_root_table: polynomials.fft_root_table,
                lookup_rows: self.lookup_rows,
                lut_to_lookups: self.lut_to_lookups,
            },
            constant_vecs: polynomials.constant_vecs,
            digest_inputs: CircuitDigestInputs {
                domain_separator,
                degree_bits: geometry.degree_bits,
            },
            success,
        }
    }

    fn circuit_digest<C: GenericConfig<D, F = F>>(
        constants_sigmas_cap: &MerkleCap<F, C::Hasher>,
        digest_inputs: &CircuitDigestInputs<F>,
    ) -> <C::Hasher as Hasher<F>>::Hash {
        let domain_separator_digest = C::Hasher::hash_pad(&digest_inputs.domain_separator);
        let circuit_digest_parts = [
            constants_sigmas_cap.flatten(),
            domain_separator_digest.to_vec(),
            vec![
                F::from_canonical_usize(digest_inputs.degree_bits),
                /* Add other circuit data here */
            ],
        ];
        C::Hasher::hash_no_pad(&circuit_digest_parts.concat())
    }

    fn finalize_cpu<C: GenericConfig<D, F = F>>(
        artifacts: CommonBuildArtifacts<F, D>,
        commitment: CpuCommitment<F, C, D>,
    ) -> (CircuitData<F, C, D>, bool) {
        let CommonBuildArtifacts {
            common,
            prover_inputs,
            constant_vecs: _constant_vecs,
            digest_inputs,
            success,
        } = artifacts;
        let constants_sigmas_cap = commitment.constants_sigmas.merkle_tree.cap.clone();
        let circuit_digest = Self::circuit_digest::<C>(&constants_sigmas_cap, &digest_inputs);
        let prover_only = ProverOnlyCircuitData::<F, C, D> {
            generators: prover_inputs.generators,
            generator_indices_by_watches: prover_inputs.generator_indices_by_watches,
            constants_sigmas_commitment: commitment.constants_sigmas,
            sigmas: transpose_poly_values(prover_inputs.sigma_vecs),
            subgroup: prover_inputs.subgroup,
            public_inputs: prover_inputs.public_inputs,
            representative_map: prover_inputs.representative_map,
            fft_root_table: Some(prover_inputs.fft_root_table),
            circuit_digest,
            lookup_rows: prover_inputs.lookup_rows.clone(),
            lut_to_lookups: prover_inputs.lut_to_lookups.clone(),
        };
    ...
    #[cfg(feature = "gpu")]
    fn finalize_gpu<C: GenericConfig<D, F = F>>(
        artifacts: CommonBuildArtifacts<F, D>,
        witness_input_targets: &[Target],
        num_ldes: [usize; 4],
    ) -> (GpuCircuitData<F, C, D>, bool) {
        let CommonBuildArtifacts {
            common,
            prover_inputs,
            constant_vecs,
        ...
        let num_const_sigmas = constant_vecs.len() + prover_inputs.sigma_vecs.len();
        let mut prover_setup = GpuProverSetup::new(&common, num_const_sigmas, num_ldes);
        let constants_sigmas_vecs = [constant_vecs, prover_inputs.sigma_vecs.clone()].concat();
        let values = constants_sigmas_vecs
            .iter()
            .map(|v| unsafe {
                mem::transmute::<PolynomialValues<F>, Vec<GoldilocksFieldBoojum>>(v.clone())
            })
            .collect::<Vec<_>>()
            .concat();
        let stream = CudaStream::default();
        memory_copy_async_w_offset(
            &mut prover_setup.parts_mut().1.fp_inputs,
            &values,
            0,
            0,
            values.len() * 8,
        ...
        prover_setup.commit_0();
        stream.synchronize().unwrap();

        let constants_sigmas_cap: MerkleCap<F, C::Hasher> = prover_setup.get_cap_device(0);
        if cfg!(feature = "verbose") {
            println!("{:?}", constants_sigmas_cap);
        }
        let circuit_digest = Self::circuit_digest::<C>(&constants_sigmas_cap, &digest_inputs);
        let prover_only = GpuProverOnlyCircuitData::<F, C, D> {
            generators: prover_inputs.generators,
            generator_indices_by_watches: prover_inputs.generator_indices_by_watches,
            sigmas: transpose_poly_values(prover_inputs.sigma_vecs),
            subgroup: prover_inputs.subgroup,
        ...
        prover_setup.init::<C>(&prover_only).unwrap();
        prover_setup.sorting_new::<C>(witness_input_targets, &common, &prover_only);

        (
            GpuCircuitData::new(prover_only, verifier_only, common, prover_setup),
            success,
        )
    }
    ...
    pub fn try_build_with_options<C: GenericConfig<D, F = F>>(
        ...
        let mut preprocessing = self.preprocess_shared::<C>(&mut timing, "sigma_ref.json");
        let commitment = Self::commit_cpu_constants_sigmas::<C>(
            &mut preprocessing,
            commit_to_sigma,
            &mut timing,
        );
        let artifacts = self.build_common_artifacts(preprocessing);
        let result = Self::finalize_cpu::<C>(artifacts, commitment);
    ...
    #[cfg(feature = "gpu")]
    pub fn build_gpu<C: GenericConfig<D, F = F>>(
        self,
        witness_input_targets: &[Target],
        num_ldes: [usize; 4],
    ) -> GpuCircuitData<F, C, D> {
        let (circuit_data, success) = self.try_build_gpu(witness_input_targets, num_ldes);
        assert!(success, "Failed to build circuit");
        circuit_data
    }

    #[cfg(feature = "gpu")]
    pub fn try_build_gpu<C: GenericConfig<D, F = F>>(
        mut self,
        witness_input_targets: &[Target],
        num_ldes: [usize; 4],
    ) -> (GpuCircuitData<F, C, D>, bool) {
        ...
        #[cfg(feature = "std")]
        let start = Instant::now();
        let preprocessing = self.preprocess_shared::<C>(&mut timing, "sigma.json");
        let artifacts = self.build_common_artifacts(preprocessing);
        let result = Self::finalize_gpu::<C>(artifacts, witness_input_targets, num_ldes);
