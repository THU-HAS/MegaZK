...
use crate::util::{debug_dump, reverse_index_bits_in_place};
...
pub fn fri_proof<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    ...
    println!("  commit phase: {:?}", s.elapsed().as_millis());
    debug_dump::write_text("fri_final_coeffs_ref.json", || {
        format!("{:?}", final_coeffs)
    });
    debug_dump::write_text("pow_sponge_ref.json", || {
        format!("{:?}", &challenger.sponge_state)
    });
    debug_dump::write_text("pow_input_buffer_ref.json", || {
        format!("{:?}", &challenger.input_buffer)
    });
    ...
    println!("  pow phase: {:?}", s.elapsed().as_millis());
    if cfg!(feature = "verbose") {
        println!("pow_witness: {:?}", pow_witness);
    }
    debug_dump::write_text("pow_witness_ref.json", || format!("{:?}", pow_witness));
    // Query phase
    let s = Instant::now();
    ...
    println!("  query phase: {:?}", s.elapsed().as_millis());
    p
}
...
fn fri_committed_trees<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    ...
    for arity_bits in &fri_params.reduction_arity_bits {
        ...
        if flag {
            debug_dump::write_text("fri_tree1_ref.json", || format!("{:?}", tree));
        }
        ...
        if cfg!(feature = "verbose") {
            println!("fri_beta: {:?}", beta);
            println!(
                "challenger sponge after beta: {:x?}",
        ...
        if flag {
            println!("{}", shift);
            debug_dump::write_text("fri_coeffs1_ref.json", || format!("{:?}", coeffs));
            debug_dump::write_text("fri_leaves2_ref.json", || format!("{:?}", values));
            flag = false;
        }
    }
...
fn fri_proof_of_work<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    ...
    if cfg!(feature = "verbose") {
        println!("pow_response: {:?}", pow_response);
    }
    let leading_zeros = pow_response.to_canonical_u64().leading_zeros();
...
>(
    ...
    let rands = challenger.get_n_challenges(fri_params.config.num_query_rounds);
    if cfg!(feature = "verbose") {
        println!("rands: {:x?}", rands);
    }
    rands
        .into_par_iter()
        .map(|rand| {
