...
impl<F: RichField + Extendable<D>, const D: usize> SimpleGenerator<F, D> for WireSplitGenerator {
    ...
    fn output_targets(&self) -> Vec<Target> {
        self.gates.iter().map(|&gate| Target::wire(gate, BaseSumGate::<2>::WIRE_SUM)).collect()
    }

    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            vec![self.integer.index(num_wires, degree), self.num_limbs, self.gates.len()]
                .into_iter()
                .chain(self.gates.iter().map(|&gate| gate * num_wires))
        );
    }
