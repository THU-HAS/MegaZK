    ...
    for QuotientGeneratorExtension<D>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        self.quotient.to_target_array().to_vec()
    }

    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            self.numerator.to_target_array().iter()
                .chain(self.denominator.to_target_array().iter())
                .chain(self.quotient.to_target_array().iter())
                .map(|t| t.index(num_wires, degree))
        );
    }
