...
#[cfg(feature = "gpu")]
use cudart::{device::set_device, memory::memory_get_info};
#[cfg(not(feature = "gpu"))]
use eyre::eyre;
use eyre::Result as EyreResult;
use log::Level;
#[cfg(feature = "gpu")]
use plonky2::plonk::circuit_data::GpuCircuitData;
use plonky2::{
    field::types::Field,
    iop::{
        target::Target,
...
use plonky2_gpu_bench::{
    ...
    BenchSession, MemoryProfile, RunReport, VerifyPolicy,
};
use rand::Rng as _;
use std::{iter::once, sync::atomic::Ordering, thread::sleep};
...
impl Circuit {
    ...
    fn prove(&self, input: &[i32]) -> EyreResult<Proof> {
        ...
        let compressed = proof
            .clone()
            .compress(&self.data.prover_only.circuit_digest, &self.data.common)
            .map_any()?;
        let proof_size_bytes = proof.to_bytes().len();
        let proof_size = ByteSize(proof_size_bytes as u64);
...
impl GpuCircuit {
    // options are inputs that can be changed from cli and have default values
    fn build_gpu(options: &Options, coefficients: &[i32], num_ldes: [usize; 4]) -> GpuCircuit {
        assert_eq!(coefficients.len(), options.input_size * options.output_size);
        info!(
            "Building circuit for for {}x{} matrix-vector multiplication",
            options.input_size, options.output_size
        ...
        builder.print_gate_counts(0);
        let gates = builder.num_gates();
        let data = builder.build_gpu::<C>(&inputs, num_ldes);
    ...
    fn prove(&mut self, input: &[i32]) -> EyreResult<Proof> {
        info!("Proving {} size input", input.len());
        // TODO: Look into plonky2::iop::PartialWitness
        // What's the difference between PW and W
        let inputs = input.iter().map(|&v| to_field(v)).collect::<Vec<F>>();
        let proof = self.data.prove(&inputs).map_any()?;
...
fn print_csv_row(options: &Options, report: &RunReport, proof_mem: usize, proof_size: u64) {
    println!(
        "{},{},{},{},{},{},{}",
        options.input_size,
        options.output_size,
        report.timings.build.as_secs_f64(),
        report.timings.prove.as_secs_f64(),
        proof_mem,
        proof_size,
        report.timings.verify.as_secs_f64()
    );
}
...
pub async fn main(mut rng: Rng, mut options: Options) -> EyreResult<()> {
    ...
    if !baseline {
        return Err(eyre!(
            "GPU proving is unavailable; rebuild with --features gpu"
        ));
    }
    let num_ldes = options.memory_profile.lde_counts([404, 400, 100, 16]);
    report_memory_profile(options.memory_profile, num_ldes);
    // let num_ldes = [0, 200, 0, 0];
    // let num_ldes = [0, 400, 0, 0];
    // let num_ldes = [202, 400, 0, 0];
    // let num_ldes = [404, 400, 0, 0];
    // let num_ldes = [404, 400, 100, 16];
    ...
    for output_size in output_sizes {
        ...
        let coefficients: Vec<i32> = (0..options.input_size * options.output_size)
            ...
            .collect();
        if baseline {
            let circuit = session.build(|| Circuit::build(&options, &coefficients));
            report_circuit_size(circuit.data.common.degree_bits(), circuit.gates);
            ...
            let input_values = (0..options.input_size as i32)
                ...
                .map(|_| rng.gen())
                .collect::<Vec<_>>();
            let proof = session.prove(|| circuit.prove(&input_values))?;
            let proof_mem = ALLOCATOR.peak_allocated.load(Ordering::Acquire);
            let proof_size = proof.to_bytes().len() as u64;
            report_proof_size(proof_size as usize);
            report_public_input_count(proof.public_inputs.len());
            info!("Prover memory usage: {}", ByteSize(proof_mem as u64));

            // Verifying
            session.verify(VerifyPolicy::Strict, || circuit.verify(&proof))?;
            let report = session.finish();
            print_csv_row(&options, &report, proof_mem, proof_size);
        } else {
            #[cfg(feature = "gpu")]
            {
                set_device(0)?;
                let mut circuit =
                    session.build(|| GpuCircuit::build_gpu(&options, &coefficients, num_ldes));
