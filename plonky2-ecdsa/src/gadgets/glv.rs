    ...
    for GLVDecompositionGenerator<F, D>
{
    ...
    fn dependencies(&self) -> Vec<Target> {
        self.k.value.limbs.iter().map(|l| l.0).rev().collect()
    }
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        add_limbs_to_output(&self.k1.value, &mut output_list);
        add_limbs_to_output(&self.k2.value, &mut output_list);

        output_list.push(self.k1_neg.target);
        output_list.push(self.k2_neg.target);
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            self.k.value.limbs.iter().map(|l| l.0.index(num_wires, degree))
                .chain(self.k1.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.k2.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain([self.k1_neg.target, self.k2_neg.target].iter().map(|t| t.index(num_wires, degree)))
        );
    }
    ...
    fn run_once(&self, witness: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
        ...
        // assert_eq!(self.k.value.num_limbs(), 8, "k limbs length: {}", self.k.value.num_limbs());
        // assert_eq!(self.k1.value.num_limbs(), 4, "k1 limbs length: {}", self.k1.value.num_limbs());
        // assert_eq!(self.k2.value.num_limbs(), 4, "k2 limbs length: {}", self.k2.value.num_limbs());
