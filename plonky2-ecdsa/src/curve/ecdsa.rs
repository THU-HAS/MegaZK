...
pub fn sign_message<C: Curve>(msg: C::ScalarField, sk: ECDSASecretKey<C>) -> ECDSASignature<C> {
    sign_message_with_rng(msg, sk, &mut OsRng)
}

pub fn sign_message_with_rng<C: Curve, R: RngCore + ?Sized>(
    msg: C::ScalarField,
    sk: ECDSASecretKey<C>,
    rng: &mut R,
) -> ECDSASignature<C> {
    let (k, rr) = {
        let mut k = C::ScalarField::sample(rng);
        let mut rr = (CurveScalar(k) * C::GENERATOR_PROJECTIVE).to_affine();
        while rr.x == C::BaseField::ZERO {
            k = C::ScalarField::sample(rng);
