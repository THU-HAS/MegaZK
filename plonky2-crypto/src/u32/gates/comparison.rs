    ...
    for ComparisonGenerator<F, D>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        let mut output_list = vec![local_target(self.gate.wire_result_bool())];

        for i in 0..self.gate.num_chunks {
            output_list.extend_from_slice(&[
                local_target(self.gate.wire_first_chunk_val(i)),
                local_target(self.gate.wire_second_chunk_val(i)),
                local_target(self.gate.wire_equality_dummy(i)),
                local_target(self.gate.wire_chunks_equal(i)),
                local_target(self.gate.wire_intermediate_value(i)),
            ]);
        }

        output_list.push(local_target(self.gate.wire_most_significant_diff()));

        for i in 0..(self.gate.chunk_bits() + 1) {
            output_list.push(local_target(self.gate.wire_most_significant_diff_bit(i)));
        }
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend_from_slice(&[self.row * num_wires, self.gate.num_bits, self.gate.num_chunks]);
    }

    fn get_params(&self, representative_map: &Vec<usize>, num_wires: usize, degree: usize) -> Vec<usize> {
        vec![self.gate.num_bits, self.gate.num_chunks]
    }
