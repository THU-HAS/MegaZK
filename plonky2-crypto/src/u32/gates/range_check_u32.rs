    ...
    for U32RangeCheckGenerator<F, D>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        (0..num_input_limbs)
            .flat_map(|i| {
            (0..self.gate.aux_limbs_per_input_limb())
                .map(move |j| Target::wire(self.row, self.gate.wire_ith_input_limb_jth_aux_limb(i, j)))
            })
            .collect()
    }

    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend_from_slice(&[self.row * num_wires, self.gate.num_input_limbs]);
    }

    fn get_params(&self, representative_map: &Vec<usize>, num_wires: usize, degree: usize) -> Vec<usize> {
        vec![self.gate.num_input_limbs]
    }
