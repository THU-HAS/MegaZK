    ...
    for U32AddManyGenerator<F, D>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        (0..(U32AddManyGate::<F, D>::num_result_limbs() + U32AddManyGate::<F, D>::num_carry_limbs()))
            .map(|j| local_target(self.gate.wire_ith_output_jth_limb(self.i, j)))
            .chain([
                local_target(self.gate.wire_ith_output_carry(self.i)),
                local_target(self.gate.wire_ith_output_result(self.i)),
            ])
            .collect()
    }

    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend_from_slice(&[
            self.row * num_wires,
            self.i,
            self.gate.num_addends,
            self.gate.num_ops,
        ]);
    }

    fn get_params(&self, representative_map: &Vec<usize>, num_wires: usize, degree: usize) -> Vec<usize> {
        vec![self.gate.num_addends]
    }
