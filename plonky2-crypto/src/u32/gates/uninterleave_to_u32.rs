    ...
    for UninterleaveToU32Generator
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        (0..num_bits / 2)
            .flat_map(|j| {
                vec![
                    local_target(2 * j + start_bits + num_bits * self.i),
                    local_target(2 * j + 1 + start_bits + num_bits * self.i),
                ]
            })
            .chain([
                local_target(self.gate.wire_ith_x_evens(self.i)),
                local_target(self.gate.wire_ith_x_odds(self.i)),
            ])
            .collect()
    }

    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend_from_slice(&[self.row * num_wires, self.i, self.gate.num_ops]);
    }
