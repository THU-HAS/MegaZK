...
impl<F: PrimeField> GeneratedValuesBigUint<F> for GeneratedValues<F> {
    fn set_biguint_target(&mut self, target: &BigUintTarget, value: &BigUint) {
        let mut limbs: Vec<u32> = value.to_u32_digits();
        assert!(target.num_limbs() >= limbs.len());
        limbs.resize(target.num_limbs(), 0);
        for (i, &limb) in limbs.iter().enumerate() {
    ...
    for BigUintDivRemGenerator<F, D>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        add_limbs_to_output(&self.rem, &mut output_list);
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            self.a.limbs.iter().map(|l| l.0.index(num_wires, degree))
                .chain(self.b.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.div.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.rem.limbs.iter().map(|l| l.0.index(num_wires, degree))),
        );
    }
    ...
    fn run_once(&self, witness: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
        ...
        // assert_eq!(self.a.num_limbs(), 16, "a limbs length: {}", self.a.num_limbs());
        // assert_eq!(self.b.num_limbs(), 8, "b limbs length: {}", self.b.num_limbs());
        // assert_eq!(self.div.num_limbs(), 9, "div limbs length: {}", self.div.num_limbs());
        // assert_eq!(self.rem.num_limbs(), 8, "rem limbs length: {}", self.rem.num_limbs());
