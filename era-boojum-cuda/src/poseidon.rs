...
pub fn build_merkle_tree_w_len<P: PoseidonImpl>(
    values: &DeviceSlice<GoldilocksField>,
    results: &mut DeviceSlice<GoldilocksField>,
    values_len: usize,
    log_rows_per_hash: u32,
    ...
    assert!(1 << (layers_count - 1) <= leaves_count);
    assert_eq!(values_len % leaves_count, 0);
    let (tree_values, _) = values.split_at(values_len);
    let (nodes, nodes_remaining) = results.split_at_mut(results.len() >> 1);
    let s = Instant::now();
    build_merkle_tree_leaves::<P>(tree_values, nodes, log_rows_per_hash, false, false, stream)?;
    stream.synchronize().unwrap();
    println!("  leaves: {:?}", s.elapsed());
    let s = Instant::now();
    build_merkle_tree_nodes::<P>(nodes, nodes_remaining, layers_count - 1, stream)?;
...
mod tests {
    ...
    fn poseidon_single_thread_leaves() {
        test_leaves::<Poseidon>(launch_single_thread_leaves_kernel::<Poseidon>, false);
    }
    //
    // #[test]
