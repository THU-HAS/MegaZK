...
multi_stage_kernel!(n2b_final_7_stages_warp_transpose);
multi_stage_kernel!(n2b_final_8_stages_warp);
multi_stage_kernel!(n2b_final_8_stages_warp_transpose);
multi_stage_kernel!(n2b_final_9_to_12_stages_block);
multi_stage_kernel!(n2b_final_9_to_12_stages_block_transpose);
...
pub fn batch_ntt_out_of_place(
    ...
    let inputs_matrix_ptr = inputs_matrix.as_mut_ptr();
    ...
    println!("  ntt internal: {:?}", s.elapsed());

    let s = Instant::now();
    batch_reverse_index_bits_inplace(
        outputs_matrix_ptr,
        log_n,
        num_ntts,
        stream
...
// #[allow(clippy::too_many_arguments)]
// pub fn batch_lde_transpose_internal(
//     inputs_ptr_in: *const GoldilocksField,
//     outputs_ptr: *mut GoldilocksField,
//     log_n: u32,
//     num_ntts: u32,
...
#[allow(clippy::too_many_arguments)]
pub fn batch_lde_transpose_out_of_place(
    ...
    let inputs_ptr = unsafe { inputs_matrix.as_ptr().add(inputs_offset as usize) };
    ...
    // ).unwrap();
    // stream.synchronize().unwrap();
    // println!("  transpose: {:?}", s.elapsed());

    get_last_error().wrap()
}

pub fn batch_lde_transpose_out_of_place_ptr(
