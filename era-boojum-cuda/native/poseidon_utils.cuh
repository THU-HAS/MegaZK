...
static DEVICE_FORCEINLINE void single_thread_leaves_impl(const base_field *values, base_field *results, const unsigned rows_count, const unsigned cols_count,
  ...
  if (load_intermediate) {
    auto intermediate_results = results + gid * CAPACITY;
#pragma unroll
    for (unsigned i = 0; i < CAPACITY; i++, intermediate_results += 1) // not enough
      state[i] = base_field::into<3>(memory::load_cs(intermediate_results));
    intermediate_results += gridDim.x * blockDim.x * CAPACITY - CAPACITY;
#pragma unroll
    for (unsigned i = CAPACITY; i < STATE_WIDTH - CAPACITY; i++, intermediate_results += 1) // not enough
      state[i] = base_field::into<3>(memory::load_cs(intermediate_results));
    intermediate_results += gridDim.x * blockDim.x * CAPACITY - CAPACITY;
#pragma unroll
    for (unsigned i = STATE_WIDTH - CAPACITY; i < STATE_WIDTH; i++, intermediate_results += 1) // not enough
      state[i] = base_field::into<3>(memory::load_cs(intermediate_results));
  }
...
#pragma unroll
    for (unsigned i = 0; i < RATE; i++, offset++) {
      ...
      state[i] = col < cols_count ? base_field::into<3>(memory::load_cs(values + row + col * rows_count * count)) : state[i];
    }
    if (cols_count > CAPACITY || load_intermediate) permutation(state);
  }
  results += gid * CAPACITY;
  if (store_intermediate) {
#pragma unroll
    for (unsigned i = 0; i < CAPACITY; i++, results += 1)
      memory::store_cs(results, base_field::field3_to_field2(state[i]));
    results += gridDim.x * blockDim.x * CAPACITY - CAPACITY;
    for (unsigned i = CAPACITY; i < STATE_WIDTH - CAPACITY; i++, results += 1)
      memory::store_cs(results, base_field::field3_to_field2(state[i]));
    results += gridDim.x * blockDim.x * CAPACITY - CAPACITY;
    for (unsigned i = STATE_WIDTH - CAPACITY; i < STATE_WIDTH; i++, results += 1)
      memory::store_cs(results, base_field::field3_to_field2(state[i]));
  } else {
#pragma unroll
    for (unsigned i = 0; i < CAPACITY; i++, results += 1)
...
template <void (*permutation)(poseidon_state &)>
static DEVICE_FORCEINLINE void single_thread_leaves_partial_impl(const base_field *static_input, const base_field *dynamic_input, base_field *output, 
  ...
  const unsigned lde_degree = degree << rate_bits;
  if (gid >= degree)
    return;
  poseidon_state state{};
  static_input += gid + (part << log_n);
  dynamic_input += gid;
  for (unsigned col = 0; col < num_ntts;) {
#pragma unroll
    for (unsigned i = 0; i < RATE; i++, col++) {
      if (col < num_ldes) {
        state[i] = col < num_ntts ? base_field::into<3>(memory::load_cs(static_input + col * lde_degree)) : state[i];
      } else {
        state[i] = col < num_ntts ? base_field::into<3>(memory::load_cs(dynamic_input + col * degree)) : state[i];
      }
    }
    if (num_ntts > CAPACITY) permutation(state);
  }
  output += gid * CAPACITY;
#pragma unroll
  for (unsigned i = 0; i < CAPACITY; i++, output += 1)
    memory::store_cs(output, base_field::field3_to_field2(state[i]));
}
...
template <void (*permutation)(poseidon_state &)>
static DEVICE_FORCEINLINE void single_thread_nodes_impl(const base_field *values, base_field *results, const unsigned count) {
  const unsigned gid = threadIdx.x + blockIdx.x * blockDim.x;
  if (gid >= count)
    return;
  poseidon_state state{};
  values += gid * CAPACITY * 2;
// #pragma unroll
//   for (unsigned i = 0; i < CAPACITY; i++, values += count) {
//     const auto value = memory::load_cs(values);
//     auto v2 = reinterpret_cast<const base_field *>(&value);
...
//   }
#pragma unroll
  for (unsigned i = 0; i < 2 * CAPACITY; i++, values += 1) {
