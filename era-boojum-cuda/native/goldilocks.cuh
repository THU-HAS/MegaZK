...
#pragma unroll
  ...
  DEVICE_FORCEINLINE field operator+(const field& other) const {
...
#pragma unroll
  ...
  static DEVICE_FORCEINLINE uint64_t to_canonical_u64(const field<2> x) {
    uint64_t result = *reinterpret_cast<const uint64_t *>(&x);
    if (result >= ORDER_U64) {
      result -= ORDER_U64;
    }
    return result;
