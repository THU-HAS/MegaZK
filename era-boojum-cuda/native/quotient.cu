...
extern "C" __launch_bounds__(128, 8) __global__
    void z_partial_in_place(const base_field *points_ptr, const base_field *z_h_coset_ptr, const base_field *k_is_ptr, const base_field *alphas,
...
#pragma unroll
    ...
    //   const unsigned next_lde_started = (index + rate) % nrows;
      const unsigned next_lde_started = reverse_bits((idx_r + rate) % nrows, degree_bits);
    ...
    //   const base_field x = base_field::mul(base_field::from_u64(7), memory::load_cs(points_ptr + index));
      const base_field x = base_field::mul(base_field::from_u64(7), memory::load_cs(points_ptr + idx_r));
    ...
    //                                                                            base_field::add(x, base_field::minus_one()))));
      const base_field l_0_x = base_field::mul(memory::load_cs(z_h_coset_ptr + (idx_r % rate)),
                                               base_field::inv(base_field::mul(base_field::from_u64(nrows >> quotient_degree_bits), // WARNING: rate bits!
                                                                               base_field::add(x, base_field::minus_one()))));
...
#pragma unroll
      for (unsigned k = 0; k < N_CLGS; k++) {
        base_field quotient_acc = output[index + k * nrows];
        // if (g_id == 0 && j == 3 && k == 0) printf("quotient_acc: %llu\n", quotient_acc);
        // base_field res = (acc[k] + quotient_acc * alpha_pows[k]) * z_h_coset_ptr[rate + (index % rate)];
        base_field res = (acc[k] + quotient_acc * alpha_pows[k]) * z_h_coset_ptr[rate + (idx_r % rate)];
        output[index + k * nrows] = res;
...
extern "C" __launch_bounds__(128, 8) __global__
    void z_partial_wo_ntt(const base_field *points_ptr, const base_field *z_h_coset_ptr, const base_field *k_is_ptr, const base_field *alphas,
      const base_field *wires_ptr, const base_field *zs_partial_products_ptr, const base_field *constants_ptr,
      const base_field *betas, const base_field *gammas, base_field *output, const unsigned degree_bits,
      const unsigned part, const unsigned num_wires, const unsigned num_const_sigmas, unsigned num_challenges,
      const unsigned num_routed_wires, const unsigned num_partial_products, const unsigned num_gate_constraints
    ) {
  const unsigned g_id = blockIdx.x * blockDim.x + threadIdx.x;
  const unsigned nrows = 1 << degree_bits;
...
#pragma unroll
      for (unsigned k = 0; k < N_CLGS; k++) {
        base_field quotient_acc = output[index + k * nrows * rate];
        // if (g_id == 0 && j == 3 && k == 0) printf("quotient_acc: %llu\n", quotient_acc);
        // base_field res = (acc[k] + quotient_acc * alpha_pows[k]) * z_h_coset_ptr[rate + (index % rate)];
        base_field res = (acc[k] + quotient_acc * alpha_pows[k]) * z_h_coset_ptr[rate + part];
        output[index + k * nrows * rate] = res;
        // if (g_id == 0 && j == 3 && k == 0) printf("q_value: %llu, to_add: %llu, z_h_inv: %llu\n", res, quotient_acc * alpha_pows[k], z_h_coset_ptr[rate + (index % rate)]);
      }
    }
  }
...
extern "C" __launch_bounds__(128, 8) __global__
    void noop_gate_in_place(const base_field *wires_ptr, const base_field *constants_ptr, const base_field *public_inputs_hash, base_field *output, const base_field *alphas,
...
extern "C" __launch_bounds__(128, 8) __global__
    void constant_gate_in_place(const base_field *wires_ptr, const base_field *constants_ptr, const base_field *public_inputs_hash, base_field *output, const base_field *alphas,
                       const unsigned degree_bits, const unsigned num_wires, const unsigned num_const_sigmas, const unsigned num_gate_constraints,
                       const unsigned num_selectors, const unsigned num_constants,
        const unsigned param1,
        const unsigned param2,
...
#pragma unroll
      for (unsigned k = 0; k < N_CLGS; k++) {
        ...
        output[index + k * nrows * RATE] = filter * acc[k] + quotient_acc;
      }
    }
  }
}
...
extern "C" __launch_bounds__(128, 8) __global__
    void public_input_gate_in_place(const base_field *wires_ptr, const base_field *constants_ptr, const base_field *public_inputs_hash, base_field *output, const base_field *alphas,
                           const unsigned degree_bits, const unsigned num_wires, const unsigned num_const_sigmas, const unsigned num_gate_constraints,
                           const unsigned num_selectors, const unsigned num_constants,
        const unsigned param1,
        const unsigned param2,
...
extern "C" __launch_bounds__(128, 8) __global__
    void arithmetic_gate_in_place(const base_field *wires_ptr, const base_field *constants_ptr, const base_field *public_inputs_hash, base_field *output, const base_field *alphas,
                         const unsigned degree_bits, const unsigned num_wires, const unsigned num_const_sigmas, const unsigned num_gate_constraints,
                         const unsigned num_selectors, const unsigned num_constants,
        const unsigned param1,
        const unsigned param2,
...
extern "C" __launch_bounds__(128, 8) __global__
    void u32_add_many_gate_in_place(
...
extern "C" __launch_bounds__(128, 8) __global__
    void u32_arithmetic_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void u32_interleave_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void u32_range_check_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void u32_subtraction_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void uninterleave_to_u32_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void random_access_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void arithmetic_extension_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void mul_extension_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void reducing_extension_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void poseidon_mds_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
...
extern "C" __launch_bounds__(128, 8) __global__
    void coset_interpolation_gate_in_place(
        const base_field *wires_ptr,
        const base_field *constants_ptr,
        const base_field *public_inputs_hash,
        base_field *output,
