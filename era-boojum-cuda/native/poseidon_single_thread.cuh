...
static DEVICE_FORCEINLINE void apply_mds_matrix_new(poseidon_state &state) {
  poseidon_state result{};
#pragma unroll
  for (unsigned row = 0; row < STATE_WIDTH; row++) {
    field<4> acc = field<4>::zero();
#pragma unroll
    for (unsigned i = 0; i < STATE_WIDTH; i++) {
      field<4> value = field<4>::mul_wide(MDS_MATRIX_CIRC[(i + STATE_WIDTH - row) % STATE_WIDTH], field<3>::field3_to_field2(state[i]));
      acc = field<4>::add_limbs(acc, value);
    }
    field<4> value = field<4>::mul_wide(MDS_MATRIX_DIAG[row], field<3>::field3_to_field2(state[row]));
    acc = field<4>::add_limbs(acc, value);
    result[row] = field<4>::field4_to_field3(acc);
  }
#pragma unroll
  for (unsigned row = 0; row < STATE_WIDTH; row++)
    state[row] = result[row];
...
static DEVICE_FORCEINLINE void partial_round_optimized_old(poseidon_state &state, const unsigned round) {
...
static DEVICE_FORCEINLINE void partial_round_optimized(poseidon_state &state, const unsigned round) {
  ...
  apply_non_linearity(state[0]);
  // display(state);
  state[0] = field<3>::add_limbs(state[0], base_field::into<3>(FAST_PARTIAL_ROUND_CONSTANTS[partial_round]));
  // display(state);
  base_field values[STATE_WIDTH];
#pragma unroll
  for (unsigned i = 0; i < STATE_WIDTH; i++)
    values[i] = field<3>::field3_to_field2(state[i]);
  const auto w_hats = FAST_PARTIAL_ROUND_W_HATS[partial_round];
  field<4> acc0 = base_field::mul_wide(field<2>::add_limbs(MDS_MATRIX_DIAG[0], MDS_MATRIX_CIRC[0]), values[0]);
  field<5> acc = field<4>::into<5>(acc0);
#pragma unroll
  for (unsigned i = 1; i < STATE_WIDTH; i++)
    acc = field<5>::add_limbs(acc, field<4>::into<5>(base_field::mul_wide(values[i], w_hats[i - 1])));
  state[0] = field<3>::field3_to_field2_and_carry(field<5>::field5_to_field3(acc));
  const auto vs = FAST_PARTIAL_ROUND_VS[partial_round];
#pragma unroll
  for (unsigned i = 1; i < STATE_WIDTH; i++)
    state[i] = field<3>::add_limbs(field<3>::field3_to_field2_and_carry(field<4>::field4_to_field3(base_field::mul_wide(values[0], vs[i - 1]))),
                                   base_field::into<3>(values[i]));
}

static DEVICE_FORCEINLINE void partial_rounds_init(poseidon_state &state) {
#pragma unroll
  for (unsigned i = 0; i < STATE_WIDTH; i++)
    state[i] = field<3>::add_limbs(state[i], base_field::into<3>(FAST_PARTIAL_FIRST_ROUND_CONSTANT[i]));
  // printf("partial rounds constants: ");
  // display(state);
  base_field values[STATE_WIDTH];
#pragma unroll
  for (unsigned i = 0; i < STATE_WIDTH; i++)
    values[i] = field<3>::field3_to_field2(state[i]);
#pragma unroll
  for (unsigned row = 1; row < STATE_WIDTH; row++) {
    const base_field *matrix_row = FAST_PARTIAL_ROUND_INITIAL_MATRIX[row - 1];
    field<5> acc{};
#pragma unroll
    for (unsigned col = 1; col < STATE_WIDTH; col++)
      acc = field<5>::add_limbs(acc, field<4>::into<5>(base_field::mul_wide(values[col], matrix_row[col - 1])));
    state[row] = field<3>::field3_to_field2_and_carry(field<5>::field5_to_field3(acc));
  }
}
