...
pub fn set_inputs<F: RichField + Extendable<D>, const D: usize>(
    input_targets: &mut Vec<Target>,
    inputs: &mut Vec<F>,
    msg: &[u8],
    sig: &[u8],
    pk: &[u8],
    ...
    for i in 0..msg_bits.len() {
        input_targets.push(msg_targets[i].target);
        inputs.push(F::from_bool(msg_bits[i]));
    }
    for i in 0..512 {
        input_targets.push(sig_targets[i].target);
        inputs.push(F::from_bool(sig_bits[i]));
    }
    for i in 0..256 {
        input_targets.push(pk_targets[i].target);
        inputs.push(F::from_bool(pk_bits[i]));
    }
}