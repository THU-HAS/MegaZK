    ...
    for NonNativeAdditionGenerator<F, D, FF>
{
    fn dependencies(&self) -> Vec<Target> {
        self.b
            ...
            .iter()
            .cloned()
            .chain(self.a.value.limbs.clone())
            .map(|l| l.0)
            .rev()
            .collect()
    }
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        let add_limbs_to_output = |target: &BigUintTarget, output_list: &mut Vec<Target>| {
            for i in 0..target.num_limbs() {
                output_list.push(target.get_limb(i).0);
            }
        };

        add_limbs_to_output(&self.sum.value, &mut output_list);

        output_list.push(self.overflow.target);

        output_list
    }

    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            [FF::type_map(), self.a.value.num_limbs(), self.b.value.num_limbs()]
                .into_iter()
                .chain(self.a.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.b.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.sum.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(vec![self.overflow.target.index(num_wires, degree)]),
        );
    }

    fn get_params(&self, representative_map: &Vec<usize>, num_wires: usize, degree: usize) -> Vec<usize> {
        vec![FF::type_map(), self.a.value.num_limbs(), self.b.value.num_limbs()]
    }
    ...
    for NonNativeMultipleAddsGenerator<F, D, FF>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        add_limbs_to_output(&self.sum.value, &mut output_list);
        output_list.push(self.overflow.0);
    ...
    for NonNativeSubtractionGenerator<F, D, FF>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        add_limbs_to_output(&self.diff.value, &mut output_list);
        output_list.push(self.overflow.target);
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            [FF::type_map(), self.a.value.num_limbs(), self.b.value.num_limbs()]
                ...
                .chain(self.a.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.b.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.diff.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(vec![self.overflow.target.index(num_wires, degree)]),
        );
    }
    ...
    for NonNativeMultiplicationGenerator<F, D, FF>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        add_limbs_to_output(&self.prod.value, &mut output_list);
        add_limbs_to_output(&self.overflow, &mut output_list);
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            [FF::type_map(), self.a.value.num_limbs(), self.overflow.num_limbs()]
                .into_iter()
                .chain(self.a.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.b.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.prod.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.overflow.limbs.iter().map(|l| l.0.index(num_wires, degree))),
        );
    }

    fn get_params(&self, representative_map: &Vec<usize>, num_wires: usize, degree: usize) -> Vec<usize> {
        vec![FF::type_map(), self.a.value.num_limbs(), self.overflow.num_limbs()]
    }
    ...
    for NonNativeInverseGenerator<F, D, FF>
{
    ...
    fn dependencies(&self) -> Vec<Target> {
        self.x.value.limbs.iter().map(|&l| l.0).rev().collect()
    }
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        add_limbs_to_output(&self.div, &mut output_list);
        add_limbs_to_output(&self.inv, &mut output_list);
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            [FF::type_map()]
                .into_iter()
                .chain(self.x.value.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.div.limbs.iter().map(|l| l.0.index(num_wires, degree)))
                .chain(self.inv.limbs.iter().map(|l| l.0.index(num_wires, degree))),
        );
    }
