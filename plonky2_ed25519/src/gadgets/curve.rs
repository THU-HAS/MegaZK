    ...
    for CurvePointDecompressionGenerator<F, D, C>
{
    ...
    fn output_targets(&self) -> Vec<Target> {
        ...
        add_limbs_to_output(&self.new_point.x.value, &mut output_list);
        add_limbs_to_output(&self.new_point.y.value, &mut output_list);
    ...
    fn pack_params(&self, u_params: &mut Vec<usize>, f_params: &mut Vec<F>, representative_map: &Vec<usize>, num_wires: usize, degree: usize) {
        u_params.extend(
            self.compressed.iter().map(|bt| bt.target.index(num_wires, degree))
                .chain(self.new_point.x.value.limbs.iter().map(|limb| limb.0.index(num_wires, degree)))
                .chain(self.new_point.y.value.limbs.iter().map(|limb| limb.0.index(num_wires, degree)))
                .collect::<Vec<usize>>()
        );
    }
